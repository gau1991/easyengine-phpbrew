...
Vagrant.configure("2") do |config|
  ...
  #
  # By default, we'll include the domains set up by VVV through the ee-hosts file
  # located in the www/ directory.
  #
  # Other domains can be automatically added by including a ee-hosts file containing
  # individual domains separated by whitespace in subdirectories of www/.
  if defined?(VagrantPlugins::HostsUpdater)
    # Recursively fetch the paths to all ee-hosts files under the www/ directory.
    paths = Dir[File.join(vagrant_dir, '**', 'ee-hosts')]

    # Parse the found ee-hosts files for host names.
